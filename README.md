...
is generated by an utility function before running the simulation.
...
[PSFLab](http://onemolecule.chem.uwm.edu/software). The PSF is computed for a single wavelength 
...
the list of particles, the simulation time step and the simulation final time.
...
[`cumsum`](http://docs.scipy.org/doc/numpy/reference/generated/numpy.cumsum.html) command.
...
are generated from a [Poisson process](http://en.wikipedia.org/wiki/Poisson_process) using the 
...
PyBroMo provide functions to simulate one ore multiple FRET populations ins  single step,
...
new smFRET file is generated taking photons from the different states as function of time.

Finally, the computation can be distributed on the nodes of a cluster (IPython cluster). 
